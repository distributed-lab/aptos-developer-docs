...
The standard supports wrapping any FA into corresponding Veiled Coin, enabling seamless integration with existing tokens.
...
For every veiled balance a user registers, they generate a unique key pair:
...
Both balances are encrypted with the same user's EK, ensuring underlying amounts are not exposed.
The veiled balance can store up to 128-bit amounts, which is the maximum supply of the FA.
