...
This standard allows any FA to be wrapped into a corresponding Veiled Coin, ensuring compatibility with existing tokens.
...
For every veiled coin a user registers, they generate a unique keypair:
...
The `VeiledCoinStore` is instantiated for each veiled coin asset the user has and managed by the `veiled_coin` module:
...
Veiled balances handle token amounts by splitting them into smaller units called chunks.
...
The pending balance consists of two chunks that hold all incoming transfers.
It can handle up to 2^16 64-bit transfers before requiring a rollover to the actual balance.
During this accumulation, the pending balance chunks can grow up to 48 bits each.

The actual balance consists of four chunks, supporting 128-bit values.
After any operation the actual balance should be [normalized](#normaliztion) back to 32-bit chunks to maintain efficient decryption.

The `VeiledBalance` struct from the `veiled_balance` module is used to represent both the pending and actual balances:
...
- Splitting the total amount into 32-bit chunks.
...
Only after a rollover, users are required to normalize the actual balance [manually].

### Homomorphic Encryption

The protocol utilizes Homomorphic encryption, allowing arithmetic operations on veiled balances without their decryption.
...
The diagram below shows the relationship between Veiled Coin modules:

<ThemedImage
  alt="VC Modules Relationship"
  sources={{
    light: '/docs/vc-diagram-light.png',
    dark: '/docs/vc-diagram-dark.png',
  }}
/>

Users interact with the `veiled_coin` module to perform operations on their veiled balances.
The `veiled_coin` module calls the `veiled_balance` module to manage the veiled balances and the `veiled_proof` module to verify ZKPs.
Under the hood, the `veiled_balance` module uses the `twisted_elgamal` module for operations on chunks.
...
```move filename="veiled_coin.move"
#[view]
public fun has_veiled_coin_store(user: address, token: Object<Metadata>): bool
```
...
You can also check if a user has a `VeiledCoinStore` for a specific token using the `has_veiled_coin_store` function.
...
- [Veiled Coin SDK](https://)
- [Veiled Coin Whitepaper](https://)
- [Veiled Coin Contracts](https://)
- [Veiled Coin Move Examples](https://)
